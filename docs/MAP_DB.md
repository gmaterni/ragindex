...
| `llm_provider` | `{provider, model, windowSize}` | Configurazione provider LLM attivo | `llm_provider.js:loadConfig(), saveConfig()` |
| `api_keys` | `{last_updated, providers: {<provider>: {exported_key, keys: [{name, key}]}}}` | Chiavi API per provider; `exported_key` è il nome della chiave attiva. **Solo offuscate nel seed, non cifrate in DB** | `key_store.js:fetchApiKeys(), restoreDefaultApiKeys(), _loadDefaultKeys()`; UI in `key_ui.js` |
...
## Database `RagIndexLLM_<userId>` — modelli LLM

Database Dexie dedicato alla gestione dei modelli LLM, **isolato per utente** con lo stesso `WebId.get()` di `RagIndexDB_<userId>` (creato da `llm/llm-db.js`). Separato dal DB applicativo: i modelli scoperti sono dati di laboratorio e non entrano nei backup KB/conversazioni.

| Store | Chiave primaria | Indici | Contenuto |
|-------|-----------------|--------|-----------|
| `discovered-models` | `id` = `"<provider>:<model>"` | `provider`, `model` | Modelli scoperti e validati dal test (voto ≥ 6), con `windowSize`, `elapsedMs`, `vote` |
| `selected-models` | `id` = `"<provider>:<model>"` | `provider`, `model` | Modelli scelti dall'utente: sono l'unico contenuto mostrato nell'albero LLM |

**Ciclo di vita**:
- `discovered-models` è riscritto da "Aggiorna LLM" (`commands/update-llm.js`) solo se la procedura non è interrotta; su STOP il contenuto precedente resta invariato.
- `selected-models` è riscritto da "Reset LLM" (`commands/reset-llm.js`, tutti i modelli dei `.txt`), da "Seleziona LLM" (Salva = sostituzione, Aggiungi = unione) e, al primo avvio con selezione vuota, dal reset automatico di `app_mgr.js:loadSelectedModels()`.
- Il catalogo in memoria (`llm_provider.js`) contiene solo i modelli selezionati (`ensureSelectedModels` + `applySelectionFilter`); durante Aggiorna/Reset contiene temporaneamente i modelli da testare.
...
### Modelli LLM (database separato)

```
RagIndexLLM_<userId> (Dexie, isolato per utente)
  ├── discovered-models            — modelli validati dal test (Aggiorna LLM)
  └── selected-models              — selezione utente → unico contenuto dell'albero LLM

llm_provider (settings, RagIndexDB_<userId>)   — {provider, model, windowSize} attivo
api_keys (settings, RagIndexDB_<userId>)       — chiavi API per provider
```
